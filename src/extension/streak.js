...
window.streak = {
 ...
 async getCurrentStreak() {
   try {
     ...
     if (!isTodaySolved) {
       ...
       // Create yesterday's date in UTC timezone
       ...
       if (value === true) {
         ...
         if (key.split('-').length === 3) {
           try {
             ...
             date = new Date(Date.UTC(year, month - 1, day)); // month is 0-indexed in JS Date
             
             if (!isNaN(date.getTime())) {
               // Format date to ISO string in UTC timezone (YYYY-MM-DD)
               const formattedDate = window.dateUtils.formatDateToUTCISO(date);
 ...
 /**
  * Helper function to convert a date to a streak key format using UTC
 ...
 /**
  * Helper function to convert a streak key to a Date object using UTC
 ...
 dateFromKey(key) {
   ...
   date.setUTCMonth(month - 1); // month is 0-indexed in Date
   date.setUTCDate(day);
 ...
 async cleanupOldStreakDays(userHandle) {
   try {
     ...
     // Calculate the date 3 months ago (in UTC)
 ...
 async shouldResetStreak() {
   try {
     ...
     if (todayDay === 1 && lastStreakDateObj.getUTCMonth() !== today.getUTCMonth()) {
       console.log("Reset streak COUNTER: First day of month with streak from previous month");
       return true;
     }
     
     // Calculate days difference properly using UTC
     lastStreakDateObj.setUTCHours(0, 0, 0, 0);
